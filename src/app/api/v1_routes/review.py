...
# async def get_user_reviews(user_id: int, review_service: ReviewService):
...
# async def create_review(review_request: ReviewCreateRequest, review_service: ReviewService):
#     return await review_service.create_review(review_request)


# @router.post("/review/image", response_model=ReviewImageResponse)
# async def create_review_image(review_request: ReviewImageRequest, review_service: ReviewService):
#     return await review_service.create_review_image(review_request)


# @router.patch("/review/{review_id}", response_model=ReviewsResponse)
# async def update_review(review_id: int, review_request: ReviewUpdateRequest, review_service: ReviewService):
#     return await review_service.update_review(review_id, review_request)


# @router.delete("/review/{review_id}", response_model=dict)
# async def delete_review(review_id: int, review_service: ReviewService):
...
# async def delete_all_user_reviews(user_id: int, review_service: ReviewService):
